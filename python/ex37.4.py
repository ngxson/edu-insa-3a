...
def score(s, C=C):
    ...
    #)
    Ps = [o/N if o != 0 else 1/(100*N) for o in occurences]
    L = sum(lg(Pk) for Pk in Ps)
    return L

def test(C):
    print(score('THISISACOHERENTSENTENCE', C))
    print(score('BLAHIBLAHBLOBYAAKNOWAHH', C))
    print(score('LKFJLSDFJIOJZOJMIOFJNZA', C))
    print(score('LKFJLSDFJIOJZOJMIOFJNZA'*2, C))
    print(score('LKFJLSDFJIOJZOJMIOFJNZA'*4, C))

test(load_grams('english_quadgrams.txt'))